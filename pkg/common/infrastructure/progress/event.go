...
// WaitingEvent creates a new Waiting in progress Event
func WaitingEvent(id string) Event {
	return NewEvent(id, Working, "Waiting")
}

// HealthyEvent creates a new Healthy (done) Event
func HealthyEvent(id string) Event {
	return NewEvent(id, Done, "Healthy")
}

func DoneEvent(id string) Event {
	return NewEvent(id, Done, "Done")
}
